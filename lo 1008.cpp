...
int main(){
  ...
  //  optimize();
    ...
    int t;
    cin>>t;  multiset<int>ma;
    for(int i=1;i<=t;i++){

        ll a;
        cin>>a;

        long long previous=floor(sqrt(a));
        if (previous*previous==a){
            previous-=1;
        }
        long long ori=previous +1;
        long long middle=ori+previous*previous;
      //  cout<<middle<<endl;
        if(a==middle){
            cout<<"Case "<<i<<": "<<ori << " "<<ori<<endl;
        }
        else{
            if(a<middle){
                    if(previous%2==0){
                        cout<<"Case "<<i<<": "<<ori << " "<<ori-((middle)-a)<<endl;
                    }
                    else{
                          cout<<"Case "<<i<<": "<<ori-((middle)-a)<< " "<<ori <<endl;
                    }
            }
            else{
                if(previous%2==0){

                              cout<<"Case "<<i<<": "<<ori-(a-(middle))<< " "<<ori <<endl;
                    }
                    else{
                   cout<<"Case "<<i<<": "<<ori << " "<<ori-(a-(middle))<<endl;
                    }
            }
        }
